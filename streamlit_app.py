    ...
    if gene_file and clinical_file:
        try:
            ...
            results = pd.DataFrame({
                "Sample": list(range(n)),  # integer index → no error
                "CAG": np.random.randint(15, 55, size=n)
            })

            results["Risk"] = results["CAG"].apply(risk_predict)

            # simple safe sort
            results = results.sort_values("Sample").reset_index(drop=True)
            ...
            for i in range(n):
                c.execute(
                    ...
                    (f"Sample_{i}", 30, int(results.loc[i, "CAG"]), results.loc[i, "Risk"])
                )
