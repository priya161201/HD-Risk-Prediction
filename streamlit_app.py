...
with tab1:
    ...
    if gene_file and clinical_file:
        try:
            ...
            # Simulated feature
            gene_df["CAG"] = np.random.randint(15, 55, size=len(gene_df))

            # Build results safely
            results = pd.DataFrame({
                "Sample": gene_df.index.astype(str),
                "CAG": gene_df["CAG"]
            })
            ...
            # ✅ FIXED SORT (NO ERROR)
            results["Sample_num"] = results["Sample"].str.extract(r'(\d+)')[0]
            results["Sample_num"] = pd.to_numeric(results["Sample_num"], errors="coerce").fillna(0)

            results = results.sort_values("Sample_num") \
